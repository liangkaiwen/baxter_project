...
const std::string KernelApplyPoseToPoints::kernel_name = "KernelApplyPoseToPoints";

KernelApplyPoseToPoints::KernelApplyPoseToPoints(OpenCLAllKernels & opencl_kernels)
	: cl_(opencl_kernels.getCL()),
	kernel_(opencl_kernels.getKernel(kernel_name))
{
}

void KernelApplyPoseToPoints::runKernel(cl::Buffer & buffer, Eigen::Affine3f const& pose, size_t size)
{
	try {
		// convert args
