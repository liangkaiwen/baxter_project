...
const std::string KernelAddFrameIfCompatible::kernel_name = "KernelAddFrameIfCompatible";

KernelAddFrameIfCompatible::KernelAddFrameIfCompatible(OpenCLAllKernels & opencl_kernels)
...
void KernelAddFrameIfCompatible::runKernel(
	...
	)
{
	try {
		...
		cl_float4 cl_this_volume_normal = {this_volume_normal[0], this_volume_normal[1], this_volume_normal[2], 0};
		...
		kernel_.setArg(kernel_arg++, buffer_normals_image.getBuffer());
		...
		kernel_.setArg(kernel_arg++, cl_this_volume_normal);
		kernel_.setArg(kernel_arg++, min_dot_product);
		kernel_.setArg(kernel_arg++, (cl_int)empty_always_included);
		kernel_.setArg(kernel_arg++, (cl_int)cos_weight);
