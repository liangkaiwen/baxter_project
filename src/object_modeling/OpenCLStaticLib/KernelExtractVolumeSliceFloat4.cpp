...
const std::string KernelExtractVolumeSliceFloat4::kernel_name = "KernelExtractVolumeSliceFloat4";

KernelExtractVolumeSliceFloat4::KernelExtractVolumeSliceFloat4(OpenCLAllKernels & opencl_kernels)
	: cl_(opencl_kernels.getCL()),
	kernel_(opencl_kernels.getKernel(kernel_name))
{
}

void KernelExtractVolumeSliceFloat4::runKernel(
	VolumeBuffer & volume,
	...
	)
{
	try {
        ...
        result.resize(image_size[1], image_size[0], 4, CV_32F);
