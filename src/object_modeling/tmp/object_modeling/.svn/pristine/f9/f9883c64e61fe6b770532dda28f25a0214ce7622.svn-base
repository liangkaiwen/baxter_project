...
const std::string KernelOptimizeErrorAndJacobianICP::kernel_name = "KernelOptimizeErrorAndJacobianICP";

KernelOptimizeErrorAndJacobianICP::KernelOptimizeErrorAndJacobianICP(OpenCLAllKernels & opencl_kernels)
...
void KernelOptimizeErrorAndJacobianICP::runKernel(
        ...
        )
{
	try {
		...
		// NO RESIZE HERE (avoid many allocations) (though it wouldn't really thanks to smart resize)
#if 0
		result_weighted_error.resize(rows, cols, 1, CV_32F);
		result_weighted_jacobian.resize(rows, cols, 6, CV_32F);
		result_debug_code.resize(rows, cols, 1, CV_32S);
		result_debug_huber_weight.resize(rows, cols, 1, CV_32F);
		result_debug_distance_weight.resize(rows, cols, 1, CV_32F);
#endif
